...
benchmarks! {
    where_clause {
        where
            ...
            <<T as pallet::Config>::MultiCurrency as MultiCurrency<<T as frame_system::Config>::AccountId>>::CurrencyId: From<Coooooins>,
    }
    ...
    }: _(RawOrigin::Signed(challenger.clone()), opponent.clone(), MatchStyle::Bullet, bet_asset_id.into(), bet_amount)
    verify {
    ...
    impl_benchmark_test_suite!(Chess, crate::mock::ExtBuilder::default().fund_alice_and_bob().build(), crate::mock::Test);
}
